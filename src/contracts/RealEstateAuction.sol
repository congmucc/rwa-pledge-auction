...
contract RealEstateAuction is IERC1155Receiver, ReentrancyGuard {
    ...
    error Auction_NoAuctionsInProgress();
    error Auction_AuctionEnded();
    error Auction_BidNotHighEnough();
    error Auction_CannotWithdrawHighestBid();
    error Auction_TooEarlyToEnd();
    ...
    function startAuction(uint256 tokenId, uint256 amount, bytes calldata data, uint256 startingBid)
        ...
        nonReentrant
    {
        if (s_started) revert Auction_AuctionAlreadyStarted();
        if (msg.sender != i_seller) revert Auction_OnlySellerCanCall();
    ...
    function bid() external payable nonReentrant {
        if (!s_started) revert Auction_NoAuctionsInProgress();
        if (block.timestamp >= s_endTimestamp) revert Auction_AuctionEnded();
        if (msg.value <= s_highestBid) revert Auction_BidNotHighEnough();
    ...
    function withdrawBid() external nonReentrant {
        if (msg.sender == s_highestBidder) revert Auction_CannotWithdrawHighestBid();
    ...
    function endAuction() external nonReentrant {
        if (!s_started) revert Auction_NoAuctionsInProgress();
        if (block.timestamp < s_endTimestamp) revert Auction_TooEarlyToEnd();
